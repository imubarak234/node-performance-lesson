...
// Create a horzontail line across the screen
cli.horizontalLine = () => {
...
cli.responders.stats = () => {
  ...
  let stats = {
    ...
    'Allocated Heap Used (%)': Math.round((v8.getHeapCodeStatistics().used_heap_size / v8.getHeapStatistics().total_heap_size) * 100),
    'Available Heap Allocated (%)': Math.round((v8.getHeapCodeStatistics().total_heap_size / v8.getHeapStatistics().heap_size_limit) * 100),
    ...
    if( !err && checkIds && checkIds.length > 0 ){
      ...
      checkIds.forEach( (checkId) => {
        _data.read( 'checks', checkId, (err, checkData) => {
          ...
          // Get the state, dafault to unknown
